...
nvar = size(datasimNG, 3); % Number of variables

% Loading Gaussian fields
load('BaseCaseSim.mat')
...
for i = 1:nvar
    subplot(1, nvar, i); % Arrange subplots in a single row
    imagesc(reshape(datasimNG(:, 1, i), [nx ny])); % Reshape and display
    colormap 'jet'; % Set colormap
    colorbar(); % Add colorbar
    clim([-3.5 3.5]); % Set color limits
    cbar =colorbar();
...
sample = 100;
%% Variogram
nbsimul=min(sample, size(datasimNG,2));
...
ghG_omni = cell(nbsimul, 1); lagG_omni = cell(nbsimul, 1);
ghNG_omni = cell(nbsimul, 1); lagNG_omni = cell(nbsimul, 1);
% Compute omnidirectional variogram for each realization
for i = 1:nbsimul
    [ghNG, nhNG] = GeoStatFFT(x0, datasimNG(:, i, :), icode, categ, display, rank);

    ang=0;
    tol_ang = 360;
    max_dist = round(2*max(nx,ny)/5); nbdist= max_dist;
    dist = [(0:nbdist-1);(1:nbdist)]'*(max_dist/nbdist);
    [ghNG_omni{i}, ~, lagNG_omni{i}] = GeoStatFFT_ndir(ghNG, nhNG, dist, ang, tol_ang);
end
...
for k = 1:nvar
    for j = k:nvar
        ...
        meansNG = zeros(length(lagNG_omni{1}{k, j}), nbsimul);

        % Fill quantile ranges for Gaussian fields
        for i = 1:nbsimul
            meansG(:, i) = ghG_omni{i}{k, j};
            meansNG(:, i) = ghNG_omni{i}{k, j};
        end
        ...
        % Compute quantile ranges
        lowerBound = quantile(meansNG, 0.05, 2);
        upperBound = quantile(meansNG, 0.95, 2);

        % Plot the shaded confidence interval for Gaussian fields
        fill([lagNG_omni{1}{k, j}; flipud(lagNG_omni{1}{k, j})], ...
        ...
        plot(lagNG_omni{1}{k, j}, mean(meansNG, 2), '-', ...
...
% Initialize variable to store results for each simulation
ghG = cell(nbsimul, 1); ghNG = cell(nbsimul, 1);
% Compute omnidirectional variogram for each realization
for i = 1:nbsimul
    ghNG{i} = GeoStatFFT(x0, datasimNG(:, i, :), icode, categ, display, rank);
end
...
for k = 1:nvar
    for j = k:nvar
        ...
        meansNG = zeros(length(ghNG{1}{k, j}(nx:end, ny)), nbsimul);

        % Fill quantile ranges for Gaussian fields
        for i = 1:nbsimul
            meansG(:, i) = ghG_dirX{i}{k, j};
            meansNG(:, i) = ghNG{i}{k, j}(nx, ny:end);
        end
